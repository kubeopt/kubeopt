...
def generate_execution_commands(analysis_data: dict) -> dict:
    """Generate deterministic kubectl commands from analysis data.

    Returns a dict keyed by recommendation id -> command string,
    for backward compatibility with callers that expect a dict.
    """
    from kubeopt.assessment.command_generator import generate_recommendations as _gen_recs
    recs = _gen_recs(analysis_data)
    return {r.id: r.command for r in recs if r.command}
