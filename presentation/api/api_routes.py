...
def register_api_routes(app):
    ...
    # AKS Excellence Scores API Route (alternative endpoint - data also available in main cluster route)
    @app.route('/api/clusters/<path:cluster_id>/aks-excellence-scores', methods=['GET'])
    def get_aks_excellence_scores(cluster_id: str):
        """Get real AKS Excellence scores using existing YAML standards flow"""
        try:
            logger.info(f"🎯 Getting AKS Excellence scores for cluster: {cluster_id}")
            
            # Use existing _get_analysis_data function (follows your standards flow)
            ...
            # Extract AKS Excellence scores (calculated by algorithmic_cost_analyzer.py)
            build_quality_score = analysis_data.get('build_quality_score')
            cost_excellence_score = analysis_data.get('cost_excellence_score')
            
            logger.info(f"🔍 DEBUG: build_quality_score = {build_quality_score}")
            logger.info(f"🔍 DEBUG: cost_excellence_score = {cost_excellence_score}")
            logger.info(f"🔍 DEBUG: aks_scoring_enabled = {analysis_data.get('aks_scoring_enabled')}")
            
            if build_quality_score is None or cost_excellence_score is None:
                return jsonify({
                    'status': 'error',
                    'message': 'AKS Excellence scores not yet calculated. Please run analysis first.',
                    'cluster_id': cluster_id,
                    'data_source': data_source,
                    'aks_scoring_enabled': analysis_data.get('aks_scoring_enabled'),
                    'debug': {
                        'build_quality_score': build_quality_score,
                        'cost_excellence_score': cost_excellence_score,
                        'total_keys': len(analysis_data.keys())
                    }
                }), 404
            
            # Format response using existing standards (no hardcoded values)
            response_data = {
                'status': 'success',
                'cluster_id': cluster_id,
                'data_source': data_source,
                ...
                'build_quality': {
                    ...
                    'breakdown': analysis_data.get('build_quality_breakdown', {}),
                    'details': analysis_data.get('build_quality_details', {})
                },
                'cost_excellence': {
                    'score': cost_excellence_score,
                    'breakdown': analysis_data.get('cost_excellence_breakdown', {}),
                    'details': analysis_data.get('cost_excellence_details', {})
                },
                'savings_opportunities': analysis_data.get('aks_savings_opportunities', [])
            }
            
            return jsonify(sanitize_for_json(response_data))
            
        except Exception as e:
            logger.error(f"❌ Error getting AKS Excellence scores: {e}")
            import traceback
            traceback.print_exc()
            return jsonify({
                'status': 'error',
                'message': f'Failed to get AKS Excellence scores: {str(e)}'
            }), 500
