    ...
    async updateSchedulerUI(status) {
        ...
        if (status) {
            ...
            if (isEnabled && isRunning) {
                ...
                statusBadge.title = `✅ Auto-monitoring active (1h intervals)\nNext analysis: ${new Date(nextRun).toLocaleString()}`;
            } else {
                ...
                statusBadge.title = '⏳ Auto-monitoring starting up\nClick to view details';
            }
        } else {
    ...
    async toggleScheduler() {
        try {
            ...
            if (toggleBtn) {
                ...
                const currentStatus = await this.getSchedulerStatus();
                if (currentStatus) {
                    toggleBtn.textContent = currentStatus.is_running ? 'Pause' : 'Start';
                }
            }
        }
    }
                ...
                <div class="scheduler-details">
                    <div class="mb-4 p-3 rounded border ${status.is_enabled && status.is_running ? 'bg-green-50 border-green-200' : status.is_enabled ? 'bg-yellow-50 border-yellow-200' : 'bg-gray-50 border-gray-200'}">
                        ...
                        <p class="text-sm text-gray-600">
                            ${status.is_enabled && status.is_running ? 'Continuous monitoring every hour' : 
                              status.is_enabled ? 'Monitoring service starting up' : 
                    ...
                    <div class="space-y-2 text-sm">
                        ...
                        <div class="flex justify-between">
                            ...
                            <span class="font-medium">Every 1 hour</span>
                        </div>
                        <div class="flex justify-between">
