...
from app.interface.chart_data_api import _generate_cpu_workload_chart
...
def register_api_routes(app):
    ...
    def api_chart_data():
        ...
        try:
            ...
            # Generate REAL CPU workload data with proper error handling
            logger.info(f"🔥 Starting independent CPU extraction for cluster: {cluster_id}")
            
            try:
                cpu_workload_data = extract_real_cpu_metrics(analysis_data)
                
                # Log detailed CPU extraction results
                if cpu_workload_data.get('has_high_cpu_workloads'):
                    logger.info(f"✅ CPU EXTRACTION SUCCESS: Found {cpu_workload_data['high_cpu_count']} high CPU workloads")
                    logger.info(f"🔥 CPU Details: max={cpu_workload_data['max_cpu_utilization']:.1f}%, "
                               f"avg={cpu_workload_data['average_cpu_utilization']:.1f}%, "
                               f"severity={cpu_workload_data['severity_level']}")
                else:
                    logger.info(f"✅ CPU EXTRACTION: Cluster healthy - {cpu_workload_data.get('reason', 'No high CPU workloads')}")
                
            except Exception as cpu_error:
                logger.error(f"❌ CPU EXTRACTION ERROR: {cpu_error}")
                # Provide fallback CPU metrics so UI doesn't break
                cpu_workload_data = _get_empty_cpu_metrics(f'CPU extraction failed: {str(cpu_error)}')
            
            # Build chart data with SAFE CPU awareness
            chart_data = {
                'status': 'success',
                'metrics': metrics,
                
                # SAFE CPU WORKLOAD METRICS
                'cpuWorkloadMetrics': cpu_workload_data,
                'hasCpuData': cpu_workload_data.get('has_high_cpu_workloads', False),
                'cpuAnalysisQuality': 'independent_extraction',
                'cpuStatus': cpu_workload_data.get('status', 'unknown'),
                ...
                'metadata': {
                    ...
                    'cpu_data_source': cpu_workload_data.get('data_source', 'none'),
                    'cpu_extraction_method': 'independent',
            ...
            try:
                ...
                # Enhanced HPA comparison with INDEPENDENT CPU data
                if 'hpa_comparison' in chart_generator_functions:
                    try:
                        ...
                        if hpa_comparison:
                            # ENHANCE HPA chart with INDEPENDENT CPU data
                            if cpu_workload_data.get('has_high_cpu_workloads'):
                                hpa_comparison['independent_cpu_data'] = {
                                    'has_high_cpu_alerts': True,
                                    ...
                                    'severity_level': cpu_workload_data['severity_level'],
                                    'workload_names': cpu_workload_data['workload_names']
                                }
                                logger.info(f"✅ Enhanced HPA chart with independent CPU data: {cpu_workload_data['high_cpu_count']} workloads")
                            
                            chart_data['hpaComparison'] = hpa_comparison
                            logger.info(f"✅ Generated HPA chart with CPU integration")
                        else:
                            logger.warning("⚠️ HPA comparison returned None - using CPU data only")
                    except Exception as hpa_error:
                        logger.error(f"❌ HPA comparison failed: {hpa_error}")
                        # CREATE FALLBACK HPA DATA with independent CPU
                        if cpu_workload_data.get('has_high_cpu_workloads'):
                            chart_data['hpaComparison'] = {
                                'fallback_mode': True,
                                'cpu_only_data': True,
                                'has_high_cpu_alerts': True,
                                'high_cpu_count': cpu_workload_data['high_cpu_count'],
                                ...
                                'recommendation_text': f"🔥 {cpu_workload_data['high_cpu_count']} high CPU workloads detected. Max: {cpu_workload_data['max_cpu_utilization']:.1f}%",
                                'timePoints': ['Current', 'Optimized'],
                                'cpuReplicas': [3, 2],
                                'memoryReplicas': [3, 2]
                            }
                            logger.info("✅ Created fallback HPA chart with independent CPU data")
                
                # DEDICATED CPU WORKLOAD CHART if we have high CPU data
                if cpu_workload_data.get('has_high_cpu_workloads'):
                    try:
                        chart_data['cpuWorkloadChart'] = _generate_cpu_workload_chart(cpu_workload_data['high_cpu_workloads'])
                        logger.info("✅ Generated dedicated CPU workload chart")
                    except Exception as cpu_chart_error:
                        logger.error(f"❌ CPU workload chart generation failed: {cpu_chart_error}")
...
def extract_real_cpu_metrics(analysis_data):
    """Extract REAL CPU metrics DIRECTLY - GRACEFUL HANDLING, NO BREAKING"""
    logger.info("🔥 INDEPENDENT CPU EXTRACTION: Starting direct data access")
    
    if not analysis_data:
        logger.warning("⚠️ No analysis data provided - returning empty CPU metrics")
        return _get_empty_cpu_metrics('No analysis data provided')
    
    logger.info(f"🔍 Available analysis_data keys: {list(analysis_data.keys())}")
    
    # 🎯 Method 1: Direct access to hpa_implementation (primary source)
    hpa_implementation = analysis_data.get('hpa_implementation', {})
    logger.info(f"🔍 hpa_implementation keys: {list(hpa_implementation.keys()) if hpa_implementation else 'None'}")
    
    high_cpu_hpas = hpa_implementation.get('high_cpu_hpas', [])
    if high_cpu_hpas and len(high_cpu_hpas) > 0:
        logger.info(f"✅ SUCCESS: Found {len(high_cpu_hpas)} high CPU workloads in hpa_implementation")
        return _process_direct_high_cpu_data(high_cpu_hpas, 'hpa_implementation')
    else:
        logger.info(f"ℹ️ hpa_implementation.high_cpu_hpas is empty: {len(high_cpu_hpas)} items")
    
    # 🎯 Method 2: Check workload_cpu_analysis
    workload_cpu_analysis = analysis_data.get('workload_cpu_analysis', {})
    logger.info(f"🔍 workload_cpu_analysis keys: {list(workload_cpu_analysis.keys()) if workload_cpu_analysis else 'None'}")
    
    high_cpu_workloads_ml = workload_cpu_analysis.get('high_cpu_workloads', [])
    if high_cpu_workloads_ml and len(high_cpu_workloads_ml) > 0:
        logger.info(f"✅ SUCCESS: Found {len(high_cpu_workloads_ml)} high CPU workloads in workload_cpu_analysis")
        return _process_direct_high_cpu_data(high_cpu_workloads_ml, 'workload_cpu_analysis')
    else:
        logger.info(f"ℹ️ workload_cpu_analysis.high_cpu_workloads is empty: {len(high_cpu_workloads_ml)} items")
    
    # 🎯 Method 3: Check other possible locations
    other_locations = [
        ('high_cpu_workloads', analysis_data.get('high_cpu_workloads', [])),
        ('cpu_analysis.high_cpu_workloads', analysis_data.get('cpu_analysis', {}).get('high_cpu_workloads', [])),
        ('ml_analysis.high_cpu_workloads', analysis_data.get('ml_analysis', {}).get('high_cpu_workloads', [])),
    ]
    
    for location_name, data in other_locations:
        logger.info(f"🔍 Checking {location_name}: {len(data) if isinstance(data, list) else 'not a list'}")
        if data and isinstance(data, list) and len(data) > 0:
            logger.info(f"✅ SUCCESS: Found {len(data)} high CPU workloads in {location_name}")
            return _process_direct_high_cpu_data(data, location_name)
    
    # 🎯 NO HIGH CPU WORKLOADS FOUND - THIS IS ACTUALLY GOOD!
    logger.info("⚠️ RESULT: No high CPU workloads found - cluster is operating normally")
    logger.info("🔍 DEBUG INFO (for troubleshooting only):")
    
    # Log key structure for debugging but don't fail
    _log_analysis_structure_for_debugging(analysis_data)
    
    # Return valid "no issues" structure instead of failing
    return _get_empty_cpu_metrics('No high CPU workloads detected - cluster operating normally')

def _get_empty_cpu_metrics(reason):
    """Return empty CPU metrics structure for normal/healthy clusters"""
    return {
        ...
        'data_source': 'no_high_cpu_detected',
        'status': 'healthy',
        'reason': reason
    }

def _process_direct_high_cpu_data(high_cpu_data, source_name):
    """Process high CPU data directly - GRACEFUL ERROR HANDLING"""
    logger.info(f"🔧 Processing {len(high_cpu_data) if isinstance(high_cpu_data, list) else 'unknown'} workloads from {source_name}")
    
    if not isinstance(high_cpu_data, list):
        logger.warning(f"⚠️ Expected list but got {type(high_cpu_data)} from {source_name} - returning empty metrics")
        return _get_empty_cpu_metrics(f'Invalid data type from {source_name}')
    
    if len(high_cpu_data) == 0:
        logger.info(f"ℹ️ No high CPU workloads in {source_name} - cluster is healthy")
        return _get_empty_cpu_metrics(f'No high CPU workloads in {source_name}')
    
    cpu_values = []
    workload_names = []
    processed_workloads = []
    
    for i, workload in enumerate(high_cpu_data):
        try:
            if isinstance(workload, dict):
                # Log the workload structure for debugging
                logger.debug(f"🔍 Workload {i} structure: {list(workload.keys())}")
                
                # Try multiple field names for CPU utilization
                cpu_util = (
                    workload.get('cpu_utilization') or 
                    workload.get('cpu_usage') or 
                    workload.get('cpu_percent') or 
                    workload.get('cpu_usage_pct') or 
                    workload.get('utilization') or 0
                )
                
                # Try multiple field names for workload name
                workload_name = (
                    workload.get('name') or 
                    workload.get('workload_name') or 
                    workload.get('pod_name') or 
                    workload.get('resource_name') or 
                    f"workload-{i+1}"
                )
                
                namespace = workload.get('namespace', 'unknown')
                full_name = f"{namespace}/{workload_name}"
                
                cpu_float = float(cpu_util)
                cpu_values.append(cpu_float)
                workload_names.append(full_name)
                
                processed_workloads.append({
                    'name': workload_name,
                    'namespace': namespace,
                    'full_name': full_name,
                    'cpu_utilization': cpu_float,
                    'original_data': workload
                })
                
                logger.debug(f"✅ Processed: {full_name} = {cpu_float}% CPU")
                
            elif isinstance(workload, (int, float)):
                cpu_float = float(workload)
                cpu_values.append(cpu_float)
                workload_names.append(f"workload-{i+1}")
                processed_workloads.append({
                    'name': f"workload-{i+1}",
                    'namespace': 'unknown',
                    'full_name': f"unknown/workload-{i+1}",
                    'cpu_utilization': cpu_float
                })
                
            else:
                logger.warning(f"⚠️ Workload {i} is not a dict or number: {type(workload)} - skipping")
                continue
                
        except Exception as e:
            logger.warning(f"⚠️ Error processing workload {i} from {source_name}: {e} - skipping")
            continue
    
    if not cpu_values:
        logger.info(f"ℹ️ No valid CPU values extracted from {source_name} - returning empty metrics")
        return _get_empty_cpu_metrics(f'No valid CPU values in {source_name}')
    ...
    logger.info(f"🔥 EXTRACTION SUCCESS: {len(cpu_values)} workloads, "
               f"max={max_cpu:.1f}%, avg={avg_cpu:.1f}%, severity={severity}, source={source_name}")
    
    return result

def _log_analysis_structure_for_debugging(analysis_data):
    """Log analysis data structure for debugging failed CPU extraction"""
    logger.error("🔍 DEBUGGING ANALYSIS_DATA STRUCTURE:")
    
    # Log top-level keys
    logger.error(f"📋 Top-level keys ({len(analysis_data)}): {list(analysis_data.keys())}")
    
    # Look for any keys containing 'cpu', 'workload', or 'hpa'
    cpu_related = [k for k in analysis_data.keys() if 'cpu' in k.lower()]
    workload_related = [k for k in analysis_data.keys() if 'workload' in k.lower()]
    hpa_related = [k for k in analysis_data.keys() if 'hpa' in k.lower()]
    
    logger.error(f"🔥 CPU-related keys: {cpu_related}")
    logger.error(f"💼 Workload-related keys: {workload_related}")
    logger.error(f"📈 HPA-related keys: {hpa_related}")
    
    # Log the structure of important keys
    important_keys = ['hpa_implementation', 'workload_cpu_analysis', 'hpa_recommendations']
    for key in important_keys:
        if key in analysis_data:
            value = analysis_data[key]
            if isinstance(value, dict):
                logger.error(f"🔍 {key} (dict): {list(value.keys())}")
                # Check for high_cpu data in nested dicts
                for nested_key, nested_value in value.items():
                    if 'cpu' in nested_key.lower() or 'high' in nested_key.lower():
                        logger.error(f"  └── {nested_key}: {type(nested_value)} = {nested_value if not isinstance(nested_value, list) or len(nested_value) < 5 else f'list[{len(nested_value)}]'}")
            else:
