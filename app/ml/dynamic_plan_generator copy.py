...
class CombinedDynamicImplementationGenerator:
    ...
    def _format_for_frontend(self, plan: ExtensiveImplementationPlan, analysis_results: Dict) -> Dict:
        ...
        This method is CRITICAL for ML component visibility in the UI
        """
        ...
        # === CRITICAL ML COMPONENT CHECK ===
        logger.info("🔍 STEP 3: ML FRAMEWORK COMPONENT DETECTION")
        ...
        logger.info(f"🎯 Looking for {len(ml_components)} ML framework components...")
        
        found_ml_components = {}
        missing_ml_components = []
        invalid_ml_components = {}
        
        for component_name in ml_components:
            logger.info(f"🔍 Checking {component_name}...")
            
            # Check in plan_dict
            if component_name in plan_dict:
                component_data = plan_dict[component_name]
                found_ml_components[component_name] = component_data
                
                # Validate component structure
                if isinstance(component_data, dict):
                    enabled = component_data.get('enabled', False)
                    has_data = component_data.get('dataAvailable', False)
                    ml_conf = component_data.get('ml_confidence', 'N/A')
                    ml_generated = component_data.get('improved_ml_generated', False)
                    
                    logger.info(f"   ✅ {component_name}: FOUND")
                    logger.info(f"      Type: {type(component_data)}")
                    logger.info(f"      Enabled: {enabled}")
                    logger.info(f"      Data Available: {has_data}")
                    logger.info(f"      ML Confidence: {ml_conf}")
                    logger.info(f"      ML Generated: {ml_generated}")
                    logger.info(f"      Keys: {list(component_data.keys())}")
                    
                    # Check for required fields
                    required_fields = ['enabled', 'dataAvailable']
                    ...
                    logger.error(f"   ❌ {component_name}: INVALID TYPE {type(component_data)}")
                    invalid_ml_components[component_name] = type(component_data)
                    
            else:
                logger.error(f"   ❌ {component_name}: NOT FOUND in plan_dict")
                missing_ml_components.append(component_name)
                
                # Check if it might be in a nested structure
                for key, value in plan_dict.items():
                    if isinstance(value, dict) and component_name in value:
                        logger.info(f"      🔍 Found {component_name} nested in {key}")
                        found_ml_components[component_name] = value[component_name]
                        missing_ml_components.remove(component_name)
                        break
        
        # ML Component Summary
        logger.info(f"📊 ML COMPONENT DETECTION SUMMARY:")
        logger.info(f"   Found: {len(found_ml_components)}/{len(ml_components)}")
        logger.info(f"   Found Components: {list(found_ml_components.keys())}")
        logger.info(f"   Missing: {len(missing_ml_components)} - {missing_ml_components}")
        logger.info(f"   Invalid: {len(invalid_ml_components)} - {invalid_ml_components}")
        
        # CRITICAL CHECK: If most components are missing, this is the UI issue!
        if len(found_ml_components) < 4:
            logger.error("🚨 CRITICAL UI ISSUE DETECTED!")
            logger.error(f"🚨 Only {len(found_ml_components)} ML components found in plan object")
            logger.error("🚨 This will cause ML framework components to not display in UI")
            
            # Try to find components in nested structures
            logger.info("🔍 Searching for ML components in nested structures...")
            for key, value in plan_dict.items():
                if isinstance(value, dict):
                    nested_ml_found = []
                    for ml_comp in missing_ml_components:
                        if ml_comp in value:
                            nested_ml_found.append(ml_comp)
                            found_ml_components[ml_comp] = value[ml_comp]
                            logger.info(f"   🔧 Rescued {ml_comp} from {key}")
                    
                    if nested_ml_found:
                        for comp in nested_ml_found:
                            missing_ml_components.remove(comp)
            
            # Updated summary after rescue attempt
            logger.info(f"📊 AFTER RESCUE ATTEMPT:")
            logger.info(f"   Found: {len(found_ml_components)}/{len(ml_components)}")
            logger.info(f"   Still Missing: {len(missing_ml_components)} - {missing_ml_components}")
        
        # DEBUG: Check other plan components
        logger.info("🔍 STEP 4: Standard Plan Component Check")
        ...
        found_standard_components = {}
        for comp_key, plan_attr in standard_components.items():
            if plan_attr in plan_dict:
                found_standard_components[comp_key] = plan_dict[plan_attr]
                logger.info(f"   ✅ {comp_key}: Found as {plan_attr}")
            elif hasattr(plan, plan_attr):
                found_standard_components[comp_key] = getattr(plan, plan_attr)
                logger.info(f"   ✅ {comp_key}: Found as attribute {plan_attr}")
            else:
                logger.warning(f"   ⚠️ {comp_key}: Not found")
        
        # === START FRONTEND RESPONSE CONSTRUCTION ===
        logger.info("🔍 STEP 5: Frontend Response Construction")
        
        # Convert phases to proper format
        logger.info("🔍 STEP 5.1: Phase Formatting")
        formatted_phases = []
        
        phases_data = found_standard_components.get('phases', [])
        if not phases_data and hasattr(plan, 'phases'):
            phases_data = plan.phases
            logger.info("📋 Using plan.phases attribute for phase data")
        ...
        logger.info("🔍 STEP 5.2: Command Generation")
        commands = []
        ...
        logger.info("🔍 STEP 5.3: Cluster Intelligence Processing")
        ...
        frontend_response = {
            'metadata': {
                ...
                'version': '2.1.0-config-enhanced-debug',
                'config_enhanced': getattr(plan, 'config_enhanced', False),
                'ml_components_detected': len(found_ml_components),  # DEBUG: Track ML components
        ...
        for response_field, plan_field in standard_response_fields.items():
            ...
            elif plan_field in plan_dict:
                frontend_response[response_field] = plan_dict[plan_field]
                logger.info(f"   ✅ Added {response_field} from plan_dict[{plan_field}]")
            else:
        ...
        # === CRITICAL: ADD ML FRAMEWORK COMPONENTS ===
        logger.info("🔍 STEP 7: Adding ML Framework Components - CRITICAL FOR UI")
        
        ml_components_added = []
        ml_components_failed = []
        
        for component_name, component_data in found_ml_components.items():
            try:
                frontend_response[component_name] = component_data
                ml_components_added.append(component_name)
                logger.info(f"   ✅ Added ML component: {component_name}")
                
                # Validate the added component
                if isinstance(component_data, dict):
                    enabled = component_data.get('enabled', False)
                    ml_conf = component_data.get('ml_confidence', 'N/A')
                    logger.info(f"      Enabled: {enabled}, Confidence: {ml_conf}")
                
            except Exception as e:
                ml_components_failed.append(component_name)
                logger.error(f"   ❌ Failed to add {component_name}: {e}")
        
        logger.info(f"📊 ML COMPONENTS ADDITION SUMMARY:")
        logger.info(f"   Successfully Added: {len(ml_components_added)} - {ml_components_added}")
        logger.info(f"   Failed to Add: {len(ml_components_failed)} - {ml_components_failed}")
        logger.info(f"   Still Missing: {len(missing_ml_components)} - {missing_ml_components}")
        
        # === EMERGENCY ML COMPONENT CREATION ===
        if len(ml_components_added) < 6:  # If we have fewer than 6 components
            logger.warning("🆘 EMERGENCY: Creating missing ML components for UI compatibility")
            
            for missing_comp in missing_ml_components:
                try:
                    emergency_component = {
                        'enabled': True,
                        'dataAvailable': True,
                        'ml_confidence': 0.7,
                        'improved_ml_generated': False,
                        'emergency_created': True,
                        'emergency_reason': 'Created in _format_for_frontend for UI compatibility',
                        'created_timestamp': datetime.now().isoformat()
                    }
                    
                    # Add component-specific emergency data
                    if missing_comp == 'costProtection':
                        emergency_component.update({
                            'budgetLimits': {'monthlyBudget': 1000, 'emergency': True},
                            'costMonitoring': {'enabled': True, 'emergency': True}
                        })
                    elif missing_comp == 'governance':
                        emergency_component.update({
                            'governanceLevel': 'standard',
                            'approvalProcess': {'required': True, 'emergency': True}
                        })
                    elif missing_comp == 'monitoring':
                        emergency_component.update({
                            'monitoringStrategy': 'standard',
                            'realTimeMonitoring': {'enabled': True, 'emergency': True}
                        })
                    elif missing_comp == 'contingency':
                        emergency_component.update({
                            'riskLevel': 'Medium',
                            'contingencyPlan': {'available': True, 'emergency': True}
                        })
                    elif missing_comp == 'successCriteria':
                        emergency_component.update({
                            'primarySuccessMetrics': {'defined': True, 'emergency': True},
                            'successThresholds': {'minimum': 0.8, 'emergency': True}
                        })
                    elif missing_comp == 'timelineOptimization':
                        emergency_component.update({
                            'timelineAnalysis': {'completed': True, 'emergency': True},
                            'phaseBreakdown': {'phases': len(formatted_phases), 'emergency': True}
                        })
                    elif missing_comp == 'riskMitigation':
                        emergency_component.update({
                            'riskAssessment': {'completed': True, 'emergency': True},
                            'mitigationStrategies': {'defined': True, 'emergency': True}
                        })
                    elif missing_comp == 'intelligenceInsights':
                        emergency_component.update({
                    ...
                    frontend_response[missing_comp] = emergency_component
                    ml_components_added.append(missing_comp)
                    logger.info(f"   🆘 Emergency created: {missing_comp}")
                    
                except Exception as e:
                    logger.error(f"   ❌ Emergency creation failed for {missing_comp}: {e}")
        ...
        # Validate ML components in final response
        ml_in_final_response = [comp for comp in ml_components if comp in frontend_response]
        logger.info(f"📊 FINAL ML COMPONENT STATUS:")
        logger.info(f"   In Response: {len(ml_in_final_response)}/{len(ml_components)}")
        logger.info(f"   Components: {ml_in_final_response}")
        
        missing_from_response = [comp for comp in ml_components if comp not in frontend_response]
        ...
        response_size = len(response_keys)
        if response_size < 15:
            logger.warning(f"⚠️ Response seems small: {response_size} keys")
        elif response_size > 50:
            logger.warning(f"⚠️ Response seems large: {response_size} keys")
        else:
            logger.info(f"✅ Response size looks good: {response_size} keys")
        
        # Add debug metadata to response
        frontend_response['_debug_frontend_formatting'] = {
            'original_ml_components_found': len(found_ml_components),
            'emergency_components_created': len([comp for comp in ml_components_added if comp in missing_ml_components]),
            'final_ml_components_in_response': len(ml_in_final_response),
            ...
            'plan_dict_keys_count': len(plan_dict),
            'response_keys_count': len(response_keys)
        }
        
        # === SUCCESS/FAILURE ASSESSMENT ===
        if len(ml_in_final_response) >= 7:
            logger.info("✅ SUCCESS: Frontend formatting completed with excellent ML component coverage")
            logger.info("✅ UI should display ML framework components correctly")
        elif len(ml_in_final_response) >= 5:
            logger.warning("⚠️ PARTIAL SUCCESS: Most ML components included, some UI features may be limited")
        else:
            logger.error("❌ FAILURE: Poor ML component coverage, significant UI issues expected")
            logger.error("❌ UI will likely not display ML framework components properly")
