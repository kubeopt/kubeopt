...
class EnhancedMultiSubscriptionClusterManager:
    ...
    def update_cluster_analysis(self, cluster_id: str, analysis_data: dict):
        """Update cluster with analysis results - preserves implementation plan with HPA validation"""
        try:
            ...
            # **NEW: Validate and enhance HPA recommendations structure**
            if 'hpa_recommendations' in enhanced_analysis_data:
                hpa_recs = enhanced_analysis_data['hpa_recommendations']
                
                # Ensure complete HPA structure for database save
                if not self._validate_hpa_structure(hpa_recs):
                    self.logger.warning(f"⚠️ DB SAVE: Fixing incomplete HPA structure for {cluster_id}")
                    enhanced_analysis_data['hpa_recommendations'] = self._fix_hpa_structure(hpa_recs, enhanced_analysis_data)
            ...
            self.logger.info(f"✅ Updated cluster analysis with validated HPA structure: {cluster_id}")
    ...
    def _validate_hpa_structure(self, hpa_recs: dict) -> bool:
    ...
    def _fix_hpa_structure(self, hpa_recs: dict, analysis_data: dict) -> dict:
        ...
        try:
            ...
            fixed_hpa = {
                'metadata': hpa_recs.get('metadata', {
                    ...
                    'recommendations_version': '2.0-fixed'
                }),
                'summary': hpa_recs.get('summary', {
            ...
            self.logger.error(f"❌ Failed to fix HPA structure: {e}")
            return hpa_recs
